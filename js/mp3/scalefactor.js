    ...
    _proto_.parseScaleFactors = function(gr, ch){
        ...
        if (sideInfo.window_switching_flag[gr][ch] != 0 && sideInfo.block_type[gr][ch] == 2) {  
            if (sideInfo.mixed_block_flag[gr][ch] != 0) {  
                ...
                // pure SHORT block  
                sideInfo.part2_bits[gr][ch] = 18 * (length0 + length1);  
                for (sfb = 0; sfb < 6; sfb++)  
                    for (win = 0; win < 3; win++)  
                        s[win][sfb] = bitStream.getBits(length0);  
                for (sfb = 6; sfb < 12; sfb++)  
            ...
            if (gr == 0) {  
                sideInfo.part2_bits[gr][ch] = 10 * (length0 + length1) + length0;  
                ...
                if (scfsi[0] == 0) {  
                    for (sfb = 0; sfb < 6; sfb++)  
                        l[sfb] = bitStream.getBits(length0);  
                    sideInfo.part2_bits[gr][ch] += 6 * length0;  
                }  
                if (scfsi[1] == 0) {  
                    for (sfb = 6; sfb < 11; sfb++)  
                        l[sfb] = bitStream.getBits(length0);  
                    sideInfo.part2_bits[gr][ch] += 5 * length0;  
                }  
                if (scfsi[2] == 0) {  
