! function(global) {
    ...
    var Player = {
            ...
            return Player.loadFrame(index, minSize, negative).then(function(result) {
                ...
                Player.audioContext.decodeAudioData(result.arrayBuffer).then(function(buffer) { //使用旧版回调，调试时将有可能被阻塞而不执行回调
                    ...
                    if(!Player.bufferLength){
                        Player.bufferLength = Math.ceil(audioInfo.totalTime*buffer.sampleRate);
                    }
        ...
        _play: function(startTime) {
            ...
            var sourceNode = Player.audioContext.createBufferSource();
            sourceNode.buffer = Player.totalBuffer;
        ...
        _startUpdateTimeoutId: function() {
            ...
            Player.timeoutIds.updateTimeoutId = setInterval(function() {
                ...
                if(Player.audioInfo.toc && currentTime > Player.audioInfo.totalTime){
                    Player._end();
                }
        ...
        fixFileBlock: function(arrayBuffer, beginIndex, endIndex, excludeBegin, excludeEnd, offset, endFrameSize) {
            ...
            function _getExtraLength(arrayBuffer, reverse, frameSize) {
            	var audioInfo = Player.audioInfo;
                var i = 0;
                var count = 200;
                var bufferStr = '';
